...
def parseString(str):
	...
	for ch in str:
		if started:
			...
			started = True
			__add_new_sublist(results,"$")
			i = len(result)-1
			
		elif ch == u"€":
			started = True
			__add_new_sublist(results,u"€")
			i = len(result)-1
	...
	__remove_empty(results)
			
	return results

#clean up list - remove empty sublists
def __remove_empty(resulting_list):
	current = len(resulting_list)-1
	while current >= 0:
		if resulting_list[current][1] == "":
			resulting_list.pop(current)
		current -= 1
	return resulting_list
	
#currency found, add sublist to list		
def __add_new_sublist(result,currencyType):
	result.append(["",""])
	i = len(result)-1
	result[i][0] = currencyType
